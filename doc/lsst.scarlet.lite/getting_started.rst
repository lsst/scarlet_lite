...
This guide is designed to show the basic functionality to get started deblending with scarlet lite.

We import the necessary modules needed to process the data:
...
If such a catalog is not available, scarlet lite does provide a module for performing detections (see :ref:`lsst.scarlet.lite-detection`), but for now we use part of the detection catalog generated by the `LSST Science Pipelines <https://github.com/lsst>`_:
...
For example, the following code converts an image array into an RGB array that can be displayed by matplotlib

>>> # Create a sinh mapping to preserve colors using the Allard-Luptoncolor scaling algorithm:
...
An :py:class:`~lsst.scarlet.lite.Observation` is a container that holds data for a set of multi-band observations,
as well as tools to convolve an image into the observed PSF in each band.
...
The ``psf_scaling`` argument determines how the PSF will be displayed. If ``psf_scaling == None`` then the PSF is not displayed at all,
if ``psf_scalling == "same"`` then the PSF image will use the same pixel scale as the observation.
...
In general these components can have independent models with their own constraints but typically, in the LSST implementation of scarlet lite, most components are factorized into a 1D spectral array that descibes the intensity  in each band, and a 2D morphology array that describes the normalized shape of the component (see :ref:`lsst.scarlet.lite-components`).
...
.. note::
    ...
    For example, one might want to create source level constrains, like sparsity, or include a multiplicative component, like a dust map.
...
In the LSST implementation of scarlet lite, all sources are modeled as a collection of one or two :py:class:`lsst.scarlet.lite.component.FactorizedComponent` instances, meaning a component can be thought of as a morphology that has a single spectrum in all of the pixels in its model.
In production these components have a pre-defined center and a morphology that monotonically decreases from the center of the component, although neither of these restrictions are required.
...
2. There is no analytic way to initialize the bulge and disk components of a two component source. This means that some approximation must be made to initialize the bulge and disk.

The scarlet developers attempted several different initialization schemes over the years, and certainly there is a lot of theoretical space we have yet to explore, but scarlet lite has implemented two of the most useful (and fastest) methods.
...
>>> # Initialize the monotnicity operator.
...
While the initialized sources from :ref:`lsst.scarlet.lite-initialization` will have their morphologies initialized properly, blends converge much faster if the initial spectra are estimated using a least squares algorithm to simultaneously fit all of the spectra to the observations using the initial morphologies.
...
Scarlet lite is designed to support a wide variety of gradient descent optimizers, which often have a number of helper variables for acceleration or keeping tack of historical values.
In order to separate the optimizer from the initialization and model creation, sources and components are typically initialized with a base :py:class:`~lsst.scarlet.lite.Parmeter` class for each parameter (ie. ``spectrum`` and ``morph``).
But before fitting the model in a blend, a specific optimizer must be chosen and scalet lite contains two options: :py:class:`~lsst.scarlet.lite.parameter.FistaParameter` (`Beck-Teboulle 2009 <https://www.ceremade.dauphine.fr/~carlier/FISTA>`_) for using the accelerated proximal gradient method and :py:class:`~lsst.scarlet.lite.parameter.AdaproxParameter` for proximal ADAM as dscribed in `Melchior et al. 2018 <https://arxiv.org/abs/1910.10094>`_.
Because different components will have different types of parameters, which migh require different step sizes or other meta parameters, a blend must be parameterized with a function similar to the default FISTA parameterization shown below (from :py:module:`lsst.scarlet.lite.component`):
...
To display the model for the entire scene, it's convovled version, the observation, and the residual, use
