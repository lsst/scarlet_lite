...
The most impactful findings are a wrong logarithm base in the Sersic gradient (critical), a 2x scaling error on `EllipseFrame` size-parameter gradients (critical, surfaced by O-6 coverage), a broken negative-origin guard in `Box.slices`, a missing sigma scaling in the circular Gaussian gradient, an `UnboundLocalError` in the wavelet initialization fallback path, and a double-padding bug in monotonic morphology initialization.
...
| C-5  | `EllipseFrame.grad_major`/`grad_minor` 2x too large    | Critical | Fixed    | Surfaced by O-6 coverage |
...
| O-4  | FFT cache grows unboundedly                            | Low      | Fixed    | Made caching opt-in. `fft.convolve` keeps `cache=True` default (no behavior change for direct callers); `Observation.convolve` defaults to `cache=False` so ad-hoc downstream calls don't grow the kernel dicts. The 5 hot-loop full-blend sites in `Blend` and `FittedPsfBlend` opt in with `cache=True` |
| O-5  | `conserve_flux` redundant zero-setting                 | Low      | Fixed    | |
| O-6  | No tests for parametric model gradients                | Medium   | Fixed    | |
| O-7  | No tests for ADAM/Adaprox variants                     | Medium   | Fixed    | |
| K-1  | `Parameter.__copy__` loses `grad`/`prox`/`step`        | Medium   | Fixed    | |
| K-2  | Positivity skipped when `bg_thresh` is active          | Low      | Fixed    | |
| K-3  | `prox_sdss_symmetry` assumes odd dimensions            | Low      | Discarded | Mathematically correct on even shapes (geometric-center symmetry); ``prox_uncentered_symmetry`` is the integer-pixel wrapper. Docstring clarified |
...
| K-5  | `uncentered_operator` even-size logic under-tested     | Low      | Fixed    | Coverage gap surfaced bug in early-return path |
| K-6  | `CubeComponent.__deepcopy__` ignores memo              | Low      | Fixed    | |
| D-1  | C++ area pre-filter uses `>` instead of `>=`           | Medium   | Fixed    | |
| D-2  | Space-branch iterative sigma is a no-op                | Medium   | Fixed    | |
...
| D-4  | Unexplained `/2` factor in detection sigma             | Low      | Deferred | **Investigated 2026-04-30**: confirmed empirical calibration so that detection noise std ≈ 1 for the LSST 6-band case (so `peak_thresh=5` means actual 5σ). Correct for the production path but band-count- and `remove_high_freq`-dependent. **2026-05-04**: doc-only fix (inline comment + docstring caveat) landed on this ticket; the full numerical fix (proper χ² coadd + analytic noise calibration) is tracked in **DM-54860** because it requires a fuller mathematical writeup and independent review, and the detection code is not used in LSST production. See expanded D-4 entry below |
| D-5  | Biased std over masked pixels                          | Low      | Fixed    | |
| D-6  | Non-reproducible random noise                          | Low      | Fixed    | |
| D-7  | Variable shadowing in `multiband_starlet_transform`    | Low      | Fixed    | |
| D-8  | Misleading `width`/`height` names                      | Low      | Fixed    | |
| D-9  | `union` docstring says "intersection"                  | Low      | Fixed    | |
| D-10 | Detection test coverage gaps                           | Low      | Fixed    | Partial: `min_pixel_detect` covered, `remove_high_freq` already covered, space-branch covered by D-2/D-6. Edge cases deferred |
...
**Fixed:** 33 (C-1, C-2, C-3, C-4, C-5, I-1, I-2, I-3, I-4, I-6, I-7, I-8, I-9, I-10, O-1, O-2, O-3, O-4, O-5, O-6, O-7, K-1, K-2, K-5, K-6, D-1, D-2, D-5, D-6, D-7, D-8, D-9, D-10).
**Discarded:** 5 (I-5, K-3, K-4, D-3, D-11).
**Deferred:** 1 (D-4 → DM-54860).
...
### C-5. `EllipseFrame.grad_major`/`grad_minor` produce 2x the correct gradient (Critical)

**File:** `python/lsst/scarlet/lite/models/parametric.py:224-272`

Surfaced while adding the finite-difference test coverage flagged in O-6. The base expression `-2/major * _xa**2` in `grad_major` is already `d(r**2)/d(major)`, while the bases in `grad_x0`, `grad_y0`, and `grad_theta` are `(1/2) * d(r**2)/d(...)`. The post-multiplications `*= 2` (when `use_r2=True`) and `*= 1/r` (when `use_r2=False`) assume the half-r**2 convention, so both branches return 2x the correct gradient for the size parameters.

Affects every parametric fit with size parameters: `grad_gaussian2` and `grad_sersic` both feed an unscaled chained gradient into `grad_major`/`grad_minor`. The optimizer therefore took twice the intended step on `sigma_y`/`sigma_x` for the entire history of the package.

**Fix:** Change the base expressions to `-1/major * _xa**2` and `-1/minor * _yb**2`, matching the half-r**2 convention used by the other partial derivatives.
...
Re-investigation showed downstream LSST code does call `observation.convolve(...)` on per-source / per-component models at varying shapes, so the leak is real.

**Fix:** caching is now opt-in.

- `fft.convolve` / `match_kernel` / `Fourier.fft` keep `cache=True` as the default — no behavior change for any direct caller.
- `Observation.convolve` defaults to `cache=False`, drawing the boundary at the place that ad-hoc downstream callers actually use. Existing callers don't need to change.
- Five hot-loop full-blend sites opt in with `cache=True`:
  - `Blend.get_model(convolve=True)` ([blend.py:244](python/lsst/scarlet/lite/blend.py#L244))
  - `Blend._grad_log_likelihood` gradient convolve ([blend.py:264](python/lsst/scarlet/lite/blend.py#L264))
  - `Blend.fit_spectra` ([blend.py:310](python/lsst/scarlet/lite/blend.py#L310)) and `Blend.conserve_flux` ([blend.py:444](python/lsst/scarlet/lite/blend.py#L444)) — both use `mode="real"` (no-op for the cache flag) but pass `cache=True` to express intent
  - `FittedPsfBlend.fit` gradient convolve ([models/fit_psf.py:220](python/lsst/scarlet/lite/models/fit_psf.py#L220))

These five sites all convolve the full blend (constant shape), so the kernel `_fft` dicts settle at one entry each and are reused every iteration — preserving the original speed motivation for the FFT module. Tests in [tests/test_fft.py](tests/test_fft.py) and [tests/test_observation.py](tests/test_observation.py) pin both defaults.
...
Reconsidered: ``prox_sdss_symmetry`` *is* mathematically correct on even-shaped arrays — the flip-and-min produces a valid symmetry about the geometric center (a half-pixel offset between the two central pixels). The audit's "incorrect results" framing assumed integer-pixel semantics, but those belong to the ``prox_uncentered_symmetry`` wrapper, which already slices to an odd-shaped subregion. Resolution: clarify the docstring instead of rejecting even shapes.
...
Adding the missing coverage surfaced an actual bug: the `peak == cy and px == cx` early-return at line 423 calls `func(x)` on the full array, but for even-shaped arrays that delegates a half-pixel-offset symmetry to `prox_sdss_symmetry` even though the caller's intent (passing an integer `peak`) is pixel-center symmetry. Fix: tighten the early-return to also require both axes odd, so even shapes go through the slicing path that properly produces an odd-shaped subregion centered on the peak.
...
Doc-only fix on this ticket: an inline comment in [detect.py](../python/lsst/scarlet/lite/detect.py) at the `sigma = ... / 2` line explains the empirical calibration and the LSST 6-band assumption, and the `peak_thresh` / `footprint_thresh` docstrings flag the σ-unit caveat. The numerical fix is **deferred to DM-54860**, which covers two related issues:

1. The detection coadd should be a Szalay-style χ² coadd (or a full χ² coadd) instead of an SED-weighted sum.
2. The `/2` fudge factor must be replaced with an analytically-derived calibration (or absorbed by the χ² coadd) so thresholds are sigma-correct for any band count.

DM-54860 is deferred, not abandoned. Two reasons: (a) the change deserves a fuller mathematical writeup with independent review before the math is touched, and (b) `detect_footprints` is not exercised by LSST science-pipelines production runs, so other audit work takes priority.
...
- `min_pixel_detect > 1` now covered by ``test_detect_footprints_min_pixel_detect`` (single-band gets fully filtered, multi-band filters selectively).
- `remove_high_freq=True` was already exercised with peak-position checks in the second half of the existing ``test_detect_footprints`` — the audit's claim was imprecise.
- Space-based wavelet branch was substantially improved by the regression tests added for D-2 (``test_space_branch_iterates_sigma``) and D-6 (``test_space_branch_reproducible``).

Deferred as targeted follow-up work when concrete failure modes appear:

- Value-level coverage of ``test_get_wavelets`` / ``test_get_detect_wavelets`` would mostly duplicate ``test_wavelet.py``.
- Edge cases (empty images, all-NaN variance, boundary footprints) each require investigating expected behavior; better as their own tickets.
